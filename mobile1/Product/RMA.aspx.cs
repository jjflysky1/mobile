...
namespace mobile1.Product
{
    public partial class RMA : System.Web.UI.Page
    {
        ...
        private void PAGEADD(int pagecount, int nowpage)
        {
            ...
            SB.Append("<li>" + "<a href='RMA.aspx?nowpage=" + 1 + " &startdate=" + startdate.Text + "&enddate=" + enddate.Text + "&search=" + Request["search"] + "&type=" + Request["type"] + "&insort=" + Request["insort"] + "&outsort=" + Request["outsort"] + "'>" + "<span aria-hidden='true'> &laquo;</span>" + "</a>" + "<li>");
            if (pagecount == 0)
            {
                SB.Append("<li> " + "<a href='RMA.aspx?nowpage=" + 1 + " &startdate=" + startdate.Text + "&enddate=" + enddate.Text + "&search=" + Request["search"] + "&type=" + Request["type"] + "&insort=" + Request["insort"] + "&outsort=" + Request["outsort"] + "' >" + 1 + "</a>" + " <li>");
            }
            int nowlist = 0;
            if (nowpage > 5)
            {
            ...
            for (int i = nowlist; i <= pagecount - 1; i++)
            {
                ...
                if (i == nowpage)
                {
                    SB.Append("<li class='page-item active'> " + "<a href='RMA.aspx?nowpage=" + i + " &startdate=" + startdate.Text + "&enddate=" + enddate.Text + " &search=" + Request["search"] + "&type=" + Request["type"] + "&sort=" + Request["sort"] + "&insort=" + Request["insort"] + "&outsort=" + Request["outsort"] + " '>" + i + "</a>" + " <li>");
                }
                else
                {
                    SB.Append("<li> " + "<a href='RMA.aspx?nowpage=" + i + " &startdate=" + startdate.Text + "&enddate=" + enddate.Text + "&search=" + Request["search"] + "&type=" + Request["type"] + "&sort=" + Request["sort"] + "&insort=" + Request["insort"] + "&outsort=" + Request["outsort"] + "'>" + i + "</a>" + " <li>");
                }
            }

            SB.Append("<li>" + "<a href='RMA.aspx?nowpage=" + (pagecount - 1) + " &startdate=" + startdate.Text + "&enddate=" + enddate.Text + "&search=" + Request["search"] + "&type=" + Request["type"] + "&insort=" + Request["insort"] + "&outsort=" + Request["outsort"] + "'>" + "<span aria-hidden='true'> &raquo;</span>" + "</a>" + "<li>");
            SB.Append("</ul>");
        ...
        private void TBLLOAD()
        {
            ...
            string SQL2 = "select count(*) as count from Product_RMA where  CONVERT(CHAR(10), indate, 23)  between '" + startdate.Text + "' and '" + enddate.Text + "' ";

            if (DropDownList1.SelectedValue == "3")
            {
                SQL2 = "select count(*) as count from Product_RMA where to_location like '%" + Search.Text + "%' and CONVERT(CHAR(10), indate, 23)  between '" + startdate.Text + "' and '" + enddate.Text + "' ";
            }
            if (DropDownList1.SelectedValue == "4")
            {
                SQL2 = "select count(*) as count from Product_RMA where product like '%" + Search.Text + "%' and CONVERT(CHAR(10), indate, 23)  between '" + startdate.Text + "' and '" + enddate.Text + "' ";

            }
            if (DropDownList1.SelectedValue == "5")
            {
                SQL2 = "select count(*) as count from Product_RMA where serial like '%" + Search.Text + "%' and CONVERT(CHAR(10), indate, 23)  between '" + startdate.Text + "' and '" + enddate.Text + "' ";

            }

            if (Request.QueryString["insort"] == "insort")
            {
                SQL2 = "select count(*) as count from Product_RMA where outdate is null and serial like '%" + Search.Text + "%' and CONVERT(CHAR(10), indate, 23)  between '" + startdate.Text + "' and '" + enddate.Text + "' ";

            }
            if (Request.QueryString["outsort"] == "outsort")
            {
                SQL2 = "select count(*) as count from Product_RMA where outdate is not null and serial like '%" + Search.Text + "%' and CONVERT(CHAR(10), indate, 23)  between '" + startdate.Text + "' and '" + enddate.Text + "' ";
            ...
            SqlDataAdapter ADT = new SqlDataAdapter("Product_RMA_sp", DB);
        ...
        private void TBLADD(string product, string serial, string indate, string outdate, string up_user, string qty, string no, string tempno, string from_location, string to_location, string out_user)
        {
            ...
            TD.Attributes["style"] = "text-align : center; vertical-align:middle; cursor:pointer;";
            TD.Attributes.Add("Onclick", "go2('" + no.ToString() + "', '" + "RMA" + "')");
            TR.Cells.Add(TD);
        ...
        {
            Response.Redirect("RMA.aspx?nowpage=" + 1 + "&search=" + Search.Text + "&type=" + DropDownList1.SelectedValue + "&startdate=" + startdate.Text + "&enddate=" + enddate.Text);
        }
        ...
        protected void Button3_Click1(object sender, EventArgs e)
        {
            ...
            {
                Response.Redirect("RMA.aspx?sort=" + HiddenField1.Value + "&insort=" + Request["insort"] + "&outsort=" + Request["outsort"] + "&HiddenField4=1");
            }
            else
            {
                Response.Redirect("RMA.aspx?sort=" + HiddenField1.Value + "&insort=" + Request["insort"] + "&outsort=" + Request["outsort"] + "&HiddenField4=0");
            }
